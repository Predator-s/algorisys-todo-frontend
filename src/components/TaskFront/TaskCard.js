...
const TaskCard = ({ task, setShowEditTaskModal, setTaskIdforEdit, onToggleFavorite }) => {
  const taskContext = useContext(TaskContext);
  const { checkTask, deleteTask } = taskContext;
  ...
  const handleToggleFavorite = () => {
    ...
    onToggleFavorite(task._id, !isFavorite);
  };
    ...
    <div className='bg-[#21202a] px-4 py-3 rounded-2xl group'>
      <div className='flex justify-between'>
          ...
          <button onClick={handleCheck}>
            <div className='border-[4px] px-2 py-2 rounded-lg border-[#fd77a1] h-[16px] w-[16px] relative'>
              <div
                ...
                className='absolute top-[11.55%] left-[13.5%] w-[12px] h-[12px] bg-[#fd77a1]'
              ></div>
            </div>
          </button>
          <div className=''>
        ...
        <div className='flex text-white gap-6 font-bold mx-3'>
          ...
          <button
            ...
            className={isFavorite ? 'text-yellow-400' : 'text-[#c2c2c5]'}
          >
            Favorite
          </button>
        </div>
      ...
      <div className='ease-in-out duration-100 h-0 group-hover:h-[30px] flex text-[14px]'>
        <h4 className='text-[#419796] font-bold py-2 ml-10 hidden group-hover:block'>
          <span className='mr-2 text-[#fd77a1]'>Created At:</span>
          <span>{new Date(task.taskcreatedat).toUTCString()}</span>
        </h4>
        <h4 className='text-[#419796] font-bold py-2 ml-10 hidden group-hover:block'>
          <span className='mr-2 text-[#fd77a1]'>Updated At:</span>
          <span>{new Date(task.taskupdatedAt).toUTCString()}</span>
        </h4>
      </div>
    </div>
  );
